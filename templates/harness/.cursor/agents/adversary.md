...
Default model is Cursor Grok 4.6 High Fast. Every Task dispatch must pass an explicit slug. Do not inherit the parent model.

- Default: `model: cursor-grok-4.6-high-fast`. Ordinary __TEAM__ work, including reviews that have some complexity.
- Max: `model: gpt-5.6-sol-xhigh`. Only when the owner asks for max, or the review is really complex or extensive: a large cross-cutting diff, an architectural or security-sensitive change, or a second pass after a deep structural miss. Do not pick max because the slice is merely non-trivial.

Frontmatter `grok-4.6[effort=high,fast=true]` is the default pin. Max overrides it at dispatch.

A Sol XHigh streak on one issue stops after 3 dispatches. The next review uses `cursor-grok-4.6-high-fast`, even if the owner said to own the fix-and-review loop. The owner can ask for another max pass, which starts a new streak of 3. A new issue resets the count.
