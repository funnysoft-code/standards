...
Laravel lives in the app tree that owns `__BOOST_ARTISAN__`. OpenCode is the laptop process harness. Laravel Boost is the Laravel agent layer.
...
- Repo-root MCP starts Boost with `php` and `__BOOST_ARTISAN__ boost:mcp` in `opencode.json`. Do not set a working directory on that command.
